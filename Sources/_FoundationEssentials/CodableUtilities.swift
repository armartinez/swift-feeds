...
public enum CodingPathNode : Sendable {
    case root
    ...
    @inline(__always)
    public func path(byAppending key: __owned (some CodingKey)?) -> [CodingKey] {
        if let key {
...
extension DecodingError {
    static func _dataCorrupted(_ debugDescription: String, for node: CodingPathNode, _ additionalKey: (some CodingKey)?) -> Self {
        Self.dataCorrupted(.init(codingPath: node.path(byAppending: additionalKey), debugDescription: debugDescription))
    }

    static func _dataCorrupted(_ debugDescription: String, for node: CodingPathNode) -> Self {
